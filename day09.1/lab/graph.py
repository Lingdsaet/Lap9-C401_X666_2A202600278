...
def supervisor_node(state: AgentState) -> AgentState:
    ...
    1. Route sang worker nào (policy_tool_worker / retrieval_worker / human_review)
    ...
    # ── Policy / Tool keywords → policy_tool_worker ──────────────────────────
    policy_keywords = [
        "hoàn tiền", "refund", "flash sale", "license", "subscription",
        "kỹ thuật số", "cấp quyền", "access level", "level 3",
        "emergency", "khẩn cấp", "quy trình",
    ]
    ...
    # ── Retrieval keywords (SLA / ticket / tài liệu) → retrieval_worker ──────
    retrieval_keywords = ["sla", "ticket", "p1", "p2", "escalat", "tài liệu", "quy định"]
    if not needs_tool and any(kw in task for kw in retrieval_keywords):
        route = "retrieval_worker"
        route_reason = "Phát hiện từ khóa tra cứu tài liệu/SLA → retrieval_worker"

    # ── Risk / High-severity flags ────────────────────────────────────────────
    risk_keywords = ["emergency", "khẩn cấp", "2am", "không rõ nguyên nhân"]
    if any(kw in task for kw in risk_keywords):
        risk_high = True
        route_reason += " (Risk High Flagged)" if route_reason else "Risk cao → cần xem xét"

    # ── Unknown error code ERR-XXX → human_review ────────────────────────────
    if re.search(r"err[-_]\d+", task) or re.search(r"err[-_][a-z0-9]+", task):
        risk_high = True
        route = "human_review"
        route_reason = "Phát hiện mã lỗi không rõ hoặc thiếu context → human_review"

    # Fallback
    if not route_reason:
        route_reason = "Không khớp từ khóa đặc biệt → retrieval_worker (mặc định)"
...
def build_graph():
    """
    Xây dựng graph với supervisor-worker pattern (Option A — Python thuần).

    Luồng:
        supervisor → route_decision → [policy_tool | human_review → retrieval | retrieval]
                                    → synthesis
    """
