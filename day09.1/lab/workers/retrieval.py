...
def _get_embedding_fn():
    """
    Trả về embedding function. Kết quả được cache ở module level
    để tránh load lại model mỗi lần gọi.
    """
    global _embed_fn
    ...
    try:
        ...
        def embed(text: str) -> list:
            resp = _oa_client.embeddings.create(input=text, model="text-embedding-3-small")
...
def _get_collection():
    """
    Kết nối ChromaDB collection. Đảm bảo collection đã được build.
    """
    import chromadb
