...
def _call_llm_policy(task: str, context: str) -> Optional[dict]:
    """Gọi LLM để phân tích policy và trích xuất JSON."""
    prompt = '''Bạn là policy analyst. Dựa vào context, xác định policy áp dụng và các exceptions.
Trích xuất thông tin thành JSON format với các trường sau:
{
  "policy_applies": boolean, // True nếu có thể áp dụng (không dính exception), False nếu bị dính exception
  "policy_name": string, // Tên quy định/policy (ví dụ refund_policy_v4)
  "exceptions_found": [ // Các ngoại lệ được tìm thấy 
    {
      "type": string, // Phân loại (ví dụ flash_sale_exception)
      "rule": string // Quy định trích xuất
    }
  ],
  "policy_version_note": string // Bất kỳ ghi chú nào về phiên bản policy
}
Chỉ trả về JSON hợp lệ, không markdown, không text dư thừa.'''
...
def analyze_policy(task: str, chunks: list) -> dict:
    """
    Phân tích policy dựa trên context chunks (sử dụng LLM, fallback rule-based).
    """
    sources = list({c.get("source", "unknown") for c in chunks if c})
    context_text_full = "\n".join([c.get("text", "") for c in chunks])
    
    # --- LLM-based Analysis ---
    llm_result = _call_llm_policy(task, context_text_full)
    if llm_result:
        return {
            "policy_applies": llm_result.get("policy_applies", True),
            ...
            "explanation": "Analyzed via LLM-based policy check.",
        }

    # --- Rule-based Fallback ---
    task_lower = task.lower()
    context_text = context_text_full.lower()
    ...
    return {
        ...
        "explanation": "Analyzed via rule-based policy check (LLM fallback).",
    }
