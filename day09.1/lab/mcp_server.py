...
def tool_create_ticket(priority: str, title: str, description: str = "") -> dict:
    ...
    if priority not in valid_priorities:
        return {
            "error": f"Priority '{priority}' không hợp lệ. Chọn: {valid_priorities}",
        }

    short_id = uuid.uuid4().hex[:6].upper()
    mock_id = f"IT-{short_id}"
    ticket = {
        "ticket_id": mock_id,
        "priority": priority,
        "title": title,
...
if __name__ == "__main__":
    ...
    new_ticket = dispatch_tool("create_ticket", {
        "priority": "P2",
        "title": "Người dùng không nhận được email xác nhận đơn hàng",
        "description": "Khoảng 15% user báo không nhận được email sau khi thanh toán.",
    })
    print(f"  Created: {new_ticket.get('ticket_id')} | {new_ticket.get('url')}")

    # 6. Test check_access_permission Level 2 (has emergency bypass)
    ...
    # 8. Test invalid priority
    print("\n❌ Test: create_ticket với priority không hợp lệ")
    err2 = dispatch_tool("create_ticket", {"priority": "P0", "title": "Test"})
    print(f"  Error: {err2.get('error')}")

    print("\n✅ MCP server test done.")
    print("\nBonus: chạy HTTP server bằng: python mcp_server.py --serve")
